...
class DCCNoticeInput extends DCCVisual {
  constructor (text, itype, buttona, buttonb, selectList) {
    ...
    if (itype) { this.itype = itype }
    if (buttona) { this.buttona = buttona }
    if (buttonb) { this.buttonb = buttonb }
    if (selectList) { this._selectList = selectList }
  }
  ...
  connectedCallback () {
    ...
    this._labelButtonA = ''
    if (this.hasAttribute('buttona') && this.buttona) { this._labelButtonA = this.buttona } else { this._labelButtonA = DCCNoticeInput.standardButtonA[this._type] }
  ...
  set text (newValue) {
    ...
    if (this._shadow != null) { this._shadow.querySelector('#text').innerHTML = newValue }
  }
  ...
  set itype (newValue) {
    ...
    if (this._inputField != null) { this._inputField.setAttribute('type', newValue) }
  }

  addSelectList (selectList) {
    this._selectList = selectList
    if (this._listWeb != null) { this._showSelectList() }
  }
  ...
  async presentNotice () {
    ...
    if (buttonClicked.button == 'B') { result = this._labelButtonB } else {
      switch (this._type) {
        case 'input':

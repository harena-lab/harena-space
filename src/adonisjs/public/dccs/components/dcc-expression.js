...
class DCCExpression extends DCCVisual {
  ...
  async connectedCallback () {
    ...
    // <TODO> provisory solution due to message ordering
    if (!this._updated) { this.innerHTML = result }
  ...
  static compileExpression (expression) {
    ...
    do {
      matchStart = -1
      let selected = ''
      for (const el in DCCExpression.element) {
        const pos = mdfocus.search(DCCExpression.element[el])
      ...
      if (matchStart > -1) {
        ...
        switch (selected) {
          ...
          case 'arithmetic':
            ...
            while (stack.length > 0 && pri <= stack[stack.length - 1][1]) { compiled.push([DCCExpression.role.arithmetic, stack.pop()[0]]) }
          ...
          case 'closeParentheses':
            while (stack.length > 0 && stack[stack.length - 1][0] != '(') { compiled.push([DCCExpression.role.arithmetic, stack.pop()[0]]) }
            if (stack.length > 0) {
              stack.pop()
  ...
  variableUpdated (topic, message) {
    ...
    if (this._index == null) { this.innerHTML = message } else { this.innerHTML = message[this._index - 1] }
  }

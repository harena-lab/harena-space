...
class DCCCell extends DCCBase {
  ...
  connectedCallback () {
    ...
    const promises = [...undefinedProps].map(property => {
  ...
  static createSVGElement (type, row, col, space) {
    let coordinates
    if (space != null) { coordinates = space.computeCoordinates(row, col) } else { coordinates = DCCSpaceCellular.computeDefaultCoordinates(row, col) }
  ...
  repositionElement (element, col, row) {
    let coordinates
    if (this.space != null) { coordinates = this.space.computeCoordinates(row, col) } else { coordinates = DCCSpaceCellular.computeDefaultCoordinates(col, row) }
    element.setAttribute('x', coordinates.x)
    element.setAttribute('y', coordinates.y)
  }

  createIndividualInitial (row, col, props) {
    const light = this.createIndividual(row, col)
    if (props != null) {
      if (light.properties == null) { light.properties = {} }
      for (const p in props) { light.properties[p] = props[p] }
    }
    return light
  }
...
class DCCCellLight {
  constructor (dcc, element) {
    ...
    if (dcc.properties != null) {
      ...
      if (props.length !== 0) {
        if (props.length == 1 && dcc.properties.value) { this.value = dcc.properties.value } else { this.properties = dcc.properties }
      }
    }
  }
}

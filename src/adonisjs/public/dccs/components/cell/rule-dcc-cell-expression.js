...
class RuleDCCCellExpression extends RuleDCCTransition {
  ...
  computeRule (spaceState, row, col) {
    ...
    const cstate = spaceState.state[row][col]

    // transfers cell position to variables
    if (!cstate.properties) { cstate.properties = {} }
    ...
    if (this._compiled != null) {
      ...
      for (const l of this._compiled) {
        for (const c of l[1]) {
          if (c[0] == varRole) {
            if (RuleDCCCellExpression.internalVar.includes(c[1])) { c[2] = cstate.properties[c[1]] }
          }
        }
        // console.log("=== state");
      ...
      if (nr >= 0 && nr < spaceState.nrows &&
             nc >= 0 && nc < spaceState.ncols) { triggered = this._computeTransition(spaceState, row, col, nr, nc) } else {
        const difr = nr - row
        const difc = nc - col
        if (nr < 0 || nr >= spaceState.nrows) {
    ...
    if (!changeRotate) {
      ...
      if (nr >= 0 && nr < spaceState.nrows &&
        ...
        if (spaceState.state[nr][nc] != null &&
          ...
          const rotateOrigin = parseInt(spaceState.state[row][col].properties.rotate)
          const rotateTarget = parseInt(spaceState.state[nr][nc].properties.rotate)
          if (rotateOrigin > rotateTarget) { spaceState.state[row][col].properties.rotate = '' + (rotateOrigin - 45) } else if (rotateOrigin < rotateTarget) { spaceState.state[row][col].properties.rotate = '' + (rotateOrigin + 45) }

...
class DCCCellRuler extends DCCBase {
  ...
  resetTool () {
    this.inactivateTool()
    const cells = this._space.cells
    for (const rs of this._rulerSet) {
  ...
  cellClicked (event) {
    ...
    const element = DCCCell.createSVGElement('image', cell.row, cell.col, this._space)
    ...
    switch (this._state) {
      ...
      case 1: this._space.cellGrid.removeEventListener('mousemove', this.rulerMoved)
        this._rulerSet.push({
          origin: this._rulerOrigin.element,
          target: element,
  ...
  createSVGRuler (origin) {
    ...
    const contentC = document.createTextNode('0' + this._unit)
    textC.appendChild(contentC)

    const text = document.createElementNS('http://www.w3.org/2000/svg', 'text')
    text.setAttribute('fill', 'red')
    text.setAttribute('transform', scale)
    const content = document.createTextNode('0' + this._unit)

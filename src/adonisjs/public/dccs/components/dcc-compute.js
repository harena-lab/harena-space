...
class DCCCompute extends DCCBase {
  ...
  static computeInstructionObj (instruction) {
    ...
    switch (instruction.type) {
      case 'divert-script':
        let message
        if (instruction.target.startsWith('Case.')) { message = 'case/' + instruction.target.substring(5) + '/navigate' } else { message = 'knot/' + instruction.target + '/navigate' }
        console.log('=== message')
        console.log(message)
        if (instruction.parameter) { MessageBus.ext.publish(message, instruction.parameter) } else { MessageBus.ext.publish(message) }
        break
    }
  }
}

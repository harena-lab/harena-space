...
class DCCStateSelect extends DCCVisual {
  ...
  async connectedCallback () {
    ...
    if (this.hasAttribute('states')) { this._statesArr = this.states.split(',') } else if (this._hasSubscriber('dcc/request/select-parameters')) {
      ...
      if (parameters.states) { this.states = parameters.states }
      if (parameters.styles) { this.styles = parameters.styles }
    }
  ...
  set states (newStates) {
    if (newStates == null) { this._statesArr = null } else {
...
class DCCGroupSelect extends DCCBlock {
  ...
  async connectedCallback () {
    if (this.vocabularies) {
      const voc = await Context.instance.loadResource(this.vocabularies)
      const states = []
      const styles = []
      for (const v in voc.states) {
        if (voc.states[v].symbol) { states.push(voc.states[v].symbol) }
        if (voc.states[v].style) { styles.push(voc.states[v].style) }
      }
  ...
  requestParameters (topic, message) {
    ...
    if (this._groupReady) { this._answerRequests() }
  }

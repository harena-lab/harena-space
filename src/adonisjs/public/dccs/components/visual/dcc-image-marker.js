...
class DCCGroupMarker extends DCCBase {
  ...
  connectedCallback () {
    ...
    if (this.hasAttribute('label')) { this._clabel.innerHTML = this.label }
  ...
  spotSelected (topic, message) {
    if (this.hasAttribute('states')) { this._spotSelectedState(message) } else { this._spotSelectedHighlight(message) }
  }
  ...
  _spotSelectedHighlight (message) {
    if (!this._selection) {
      for (const s in this._spots) {
        this._spots[s].rect.setAttributeNS(null, 'style',
          'opacity:0;fill:#0000ff;stroke-width:0.89027536')
        if (this._spots[s].label == message.label) { this._zoomSpot(this._spots[s].coords, this._imageC, this._clipC, 'C', false) }
      }
      ...
      while (this._clipC.firstChild) { this._clipC.removeChild(this._clipC.firstChild) }

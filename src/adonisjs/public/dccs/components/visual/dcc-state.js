...
class DCCState extends DCCBlock {
  ...
  set rotate (isRotate) {
    if (isRotate) { this.setAttribute('rotate', '') } else { this.removeAttribute('rotate') }
  }
  ...
  shiftState (shift) {
    ...
    const s = allStates.indexOf(this.value)
    if (s >= 0) {
      this.changed = true
      let n = s + shift
      n = (n < 0)
  ...
  notify (topic, message) {
    if (message.role) {
      if (message.body) { this._computeAction(message.role.toLowerCase(), message.body) } else { this._computeAction(message.role.toLowerCase()) }
    }
  }

  _computeTrigger () {
    if (this.hasAttribute('action')) { this._computeAction(this.action) }
  }
  ...
  _computeAction (action, parameter) {
    switch (action.toLowerCase()) {
      ...
      case 'state': if (parameter) { this.changeState(parameter.value) }
        break
    }
  }

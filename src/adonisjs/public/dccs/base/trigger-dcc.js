...
class TriggerDCC extends PrimitiveDCC {
  connectedCallback () {
    ...
    if (this.hasAttribute('event')) {
      ...
      if (this._sourceObj != null) {
        ...
        if (this.hasAttribute('target')) { this._targetObj = document.querySelector('#' + this.target) } else { this._targetObj = null }
      }
    }
  }
  ...
  notifyTrigger (event) {
    ...
    console.log(event)
    const message = (this.hasAttribute('role')) ? { role: this.role } : {}
    if (this.hasAttribute('property') || this.hasAttribute('value')) {
      message.body = {}
      if (this.hasAttribute('property')) { message.body[this.property] = this._sourceObj[this.property] }
